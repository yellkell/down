...
export interface EnvHandles {
  ...
  finish: Group;
  /** Microscopic startup clone that uploads finish debris before gameplay. */
  finishWarmup: Group;
...
export class EnvironmentSystem extends createSystem({}) {
  ...
  update(delta: number, time: number): void {
    ...
    if (this.finishWarmupFrames > 0) {
      this.finishWarmupFrames -= 1;
    } else if (this.finishWarmupFrames === 0) {
      env.finishWarmup.removeFromParent();
      this.finishWarmupFrames = -1;
    }
